...
int MyFS::fuseRead(const char *path, char *buf, size_t size, off_t offset,
    ...
    uint16_t *blocks =
        (uint16_t *)malloc((blockOffset + blockCount) * sizeof(uint16_t));
...
int MyFS::fuseWrite(const char *path, const char *buf, size_t size,
    ...
    if (newBlockCount > 0) {
        uint16_t *newBlocks =
            (uint16_t *)malloc(newBlockCount * sizeof(uint16_t));
        ...
        if (tmpFile->firstBlock == 0) {
            ...
            int lastBlock = 0;
            for (int tmpBlock = tmpFile->firstBlock; tmpBlock != 0;
    ...
    int offsetCount = 0;
    int blockIndex = 0;
    for (int tmpBlock = tmpFile->firstBlock;
         tmpBlock != 0 && blockIndex < blockCount;
         tmpBlock = fat->read(tmpBlock)) {
